...
{
	CFile file;
	file.Open(CString(filename),CFile::modeCreate | CFile::modeWrite | CFile::shareDenyNone);
	DWORD rez=Save(file);
	file.Close();
	return rez;
}
...
BOOL CDib::Read(char* filename)
{
	CFile file;
	DWORD rez=0;
	BOOL brez=file.Open(CString(filename),CFile::modeRead | CFile::shareDenyNone);
	//myiDebug(brez);
	if(brez!=0)
	{
		rez=Read(file);
